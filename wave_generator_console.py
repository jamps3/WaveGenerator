...
class WaveGenerator:
    def __init__(self):
        ...
        pygame.mixer.pre_init(frequency=48000, size=-16, channels=2, buffer=64)
        pygame.mixer.init()
        ...
        self.sample_rate = (
            48000  # Match sample rate with pygame mixer for optimal performance
        )
        ...
        # Initialize the shared wave engine
        self.wave_engine = WaveEngine(sample_rate=self.sample_rate)
    ...
    def generate_wave(self, wave_type, frequency, volume, duration):
        ...
        return self.wave_engine.generate_wave(wave_type, frequency, volume, duration)
    ...
    def generate_wave_chunk(self, wave_type, frequency, volume, frames, start_phase):
        ...
        return self.wave_engine.generate_wave_chunk(wave_type, frequency, volume, frames, start_phase)
    ...
    def generate_fast_transition_waveform(self, old_wave_type, old_frequency, old_volume,
                                        new_wave_type, new_frequency, new_volume,
                                        start_phase, transition_frames, loop_frames):
        """Generate ultra-fast transition waveform for immediate frequency/parameter changes"""
    ...
    def generate_frequency_transition_waveform(self, old_wave_type, old_frequency, old_volume,
                                             new_wave_type, new_frequency, new_volume,
                                             start_phase, transition_frames, loop_frames):
        """Generate a seamless frequency transition waveform without any artifacts"""
        total_frames = transition_frames + loop_frames
        arr = np.zeros((total_frames, 2), dtype=np.int16)
        
        # Use numpy arrays for vectorized operations
        i_array = np.arange(total_frames, dtype=np.float64)
        
        # Create smooth frequency transition over transition_frames
        frequency_array = np.zeros(total_frames)
        
        for i in range(total_frames):
            if i < transition_frames:
                # Smooth frequency transition using cubic interpolation
                t = i / transition_frames
                # Use smoothstep for natural frequency transition
                smooth_t = t * t * (3 - 2 * t)
                frequency_array[i] = old_frequency + (new_frequency - old_frequency) * smooth_t
            else:
                # Constant new frequency after transition
                frequency_array[i] = new_frequency
        
        # Calculate instantaneous phase for each sample
        phase_array = np.zeros(total_frames)
        phase_array[0] = start_phase
        
        # Integrate frequency to get phase (ensuring phase continuity)
        for i in range(1, total_frames):
            dt = 1.0 / self.sample_rate
            phase_array[i] = phase_array[i-1] + frequency_array[i] * 2 * np.pi * dt
        
        # Generate waveform values
        wave_values = np.zeros(total_frames)
        
        # Handle wave type transitions
        for i in range(total_frames):
            if i < transition_frames:
                # Smooth transition between wave types
                t = i / transition_frames
                smooth_t = t * t * (3 - 2 * t)  # smoothstep
                
                # Generate old wave type value
                if old_wave_type == "sine":
                    old_value = np.sin(phase_array[i])
                elif old_wave_type == "square":
                    old_value = np.sign(np.sin(phase_array[i]))
                elif old_wave_type == "sawtooth":
                    normalized_phase = phase_array[i] % (2 * np.pi)
                    old_value = 2 * (normalized_phase / (2 * np.pi)) - 1
                else:
                    old_value = np.sin(phase_array[i])
                
                # Generate new wave type value
                if new_wave_type == "sine":
                    new_value = np.sin(phase_array[i])
                elif new_wave_type == "square":
                    new_value = np.sign(np.sin(phase_array[i]))
                elif new_wave_type == "sawtooth":
                    normalized_phase = phase_array[i] % (2 * np.pi)
                    new_value = 2 * (normalized_phase / (2 * np.pi)) - 1
                else:
                ...
                # Smooth transition between wave types and volumes
                old_vol_factor = (old_volume / 100.0) * (1 - smooth_t)
                new_vol_factor = (new_volume / 100.0) * smooth_t
                
                wave_values[i] = old_value * old_vol_factor + new_value * new_vol_factor
            else:
                # Pure new wave type after transition
                if new_wave_type == "sine":
                    wave_values[i] = np.sin(phase_array[i]) * (new_volume / 100.0)
                elif new_wave_type == "square":
                    wave_values[i] = np.sign(np.sin(phase_array[i])) * (new_volume / 100.0)
                elif new_wave_type == "sawtooth":
                    normalized_phase = phase_array[i] % (2 * np.pi)
                    wave_values[i] = (2 * (normalized_phase / (2 * np.pi)) - 1) * (new_volume / 100.0)
                else:
                    wave_values[i] = np.sin(phase_array[i]) * (new_volume / 100.0)
        ...
        while self.sweep_active:
            try:
                ...
                current_freq = (
                    ...
                    if self.sweep_current_frequency > 0
                    else self.sweep_start
                )
                ...
                current_phase = (
                    current_phase
                    + (chunk_frames / self.sample_rate) * current_freq * 2 * np.pi
                ) % (2 * np.pi)
    ...
    def play_continuous_sound(self):
        """Play continuous tone sound with anti-click frequency transitions"""
        try:
            # Use shorter buffer for more responsive transitions
            loop_duration = 0.1  # 100ms buffer for responsive transitions
            ...
            wave_data = self.generate_wave_chunk(
                self.wave_type, self.frequency, self.volume, loop_frames, 0.0
            )
        ...
        # Use dual-channel system for smooth crossfading
        ...
        while not self.stop_sound:
            ...
            if new_params != current_params and not self.transition_in_progress:
                ...
                try:
                    # Calculate precise current phase for seamless transition
                    current_time = time.time()
                    with self.phase_lock:
                        elapsed_time = current_time - self.playback_start_time
                        # More accurate phase calculation that handles frequency changes better
                        total_cycles = elapsed_time * old_frequency
                        current_phase = (total_cycles - np.floor(total_cycles)) * 2 * np.pi
                        
                        # Generate smooth crossfade transition (short fade to prevent clicks)
                        crossfade_duration = 0.005  # 5ms crossfade to prevent clicks
                        crossfade_frames = max(1, int(crossfade_duration * self.sample_rate))
                        
                        # Generate new sound with smooth transition
                        new_wave_data = self.create_anti_click_transition(
                            old_wave_type, old_frequency, old_volume,
                            new_wave_type, new_frequency, new_volume,
                            current_phase, crossfade_frames, loop_frames
                        )
                        new_sound = pygame.sndarray.make_sound(new_wave_data)
                        
                        # Update phase tracking for continuous playback
                        self.current_phase = current_phase
                        self.playback_start_time = current_time

                    # Smooth crossfade using dual channels
                    next_channel.set_volume(0.0)
                    next_channel.play(new_sound, loops=-1)
                    
                    # Fade out old channel and fade in new channel
                    fade_steps = 10
                    fade_duration = 0.01  # 10ms total fade
                    step_duration = fade_duration / fade_steps
                    
                    for i in range(fade_steps + 1):
                        if self.stop_sound:
                            break
                        fade_progress = i / fade_steps
                        active_channel.set_volume(1.0 - fade_progress)
                        next_channel.set_volume(fade_progress)
                        time.sleep(step_duration)
                    
                    # Complete the transition
                    active_channel.stop()
                    active_channel, next_channel = next_channel, active_channel
                    current_sound = new_sound
                    current_params = new_params
                    
                except Exception as e:
                    print(f"Error in transition: {e}")
                    # Fallback to simple replacement if crossfade fails
                    try:
                        active_channel.stop()
                        active_channel.play(new_sound, loops=-1)
                        current_params = new_params
                    except:
                        pass
            ...
            time.sleep(0.005)  # Faster checking for ultra-responsive transitions
        ...
        for i, option in enumerate(self.menu_options):
            ...
            if i == 0:  # Play/Pause
                ...
                if self.sweep_active and self.sweep_current_frequency > 0:
                    ...
                    print(
                        f"{prefix}{option}: {sweep_status} (Current: {current_display})"
                    )
                else:
    ...
    def handle_input(self):
        ...
        def on_key_event(event):
            ...
            if (
                key in last_key_time
                and (current_time - last_key_time[key]) < debounce_delay
            ):
                return
